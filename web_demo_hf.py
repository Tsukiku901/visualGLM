...
def main(args):
    ...
    tokenizer = AutoTokenizer.from_pretrained("/content/visualglm-6b", trust_remote_code=True)
    if args.quant in [4, 8]:
        model = AutoModel.from_pretrained("/content/visualglm-6b", trust_remote_code=True).quantize(
            args.quant).half().cuda()
    else:
        model = AutoModel.from_pretrained("/content/visualglm-6b", trust_remote_code=True).half().cuda()
    model = model.eval()
    ...
    with gr.Blocks(css='style.css') as demo:
        ...
        demo.queue().launch(share=args.share, inbrowser=True, server_name='0.0.0.0', server_port=7860)
